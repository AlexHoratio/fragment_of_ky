...
func process(delta: float) -> void:
	...
	fsm.kyztling.movement_vector = Vector2(1, 0).rotated(fsm.kyztling.global_position.angle_to_point(destination))
	
	#if fsm.kyztling.get_node("AnimatedSprite2D").animation != "walk":
		#fsm.kyztling.get_node("AnimatedSprite2D").animation = "walk"
	
	if fsm.kyztling.global_position.distance_to(destination) < 5:
		fsm.change_to("IDLE")
